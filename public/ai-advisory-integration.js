...
if (typeof window !== 'undefined' && window.AIAdvisoryIntegration) {
    console.log('AIAdvisoryIntegration already declared, skipping...');
} else {
class AIAdvisoryIntegration {
    ...
    async init() {
        ...
        try {
            ...
            // Check if AI is enabled - allow fallback mode
            if (!this.configManager.isAIEnabled()) {
                console.log('⚠️ AI is disabled, using fallback mode');
                // Use fallback mode instead of throwing error
                await this.initializeFallbackMode();
                return; // Exit early since fallback mode handles initialization
            }
    ...
    async initializeFallbackMode() {
        ...
        try {
            // Create advisor manager with fallback configuration
            this.advisorManager = new AdvisorManager({
                useFallbackOnly: true, // Force fallback mode
                fallbackMode: true
            });
            
            console.log('✅ AdvisorManager created with fallback config:', {
                fallbackMode: this.advisorManager.fallbackMode,
                useFallbackOnly: this.advisorManager.config.useFallbackOnly
            });
            
            // Initialize the advisor manager (await the async init)
            console.log('🔧 About to call advisorManager.init()...');
            await this.advisorManager.init();
            console.log('🔧 advisorManager.init() completed');
            
            console.log('✅ AdvisorManager initialized, provider status:', {
                hasProvider: !!this.advisorManager.provider,
                fallbackMode: this.advisorManager.fallbackMode
            });
            ...
            console.error('❌ Error in initializeFallbackMode:', error);
            throw error;
        }
    }
    ...
    waitForDashboardReady() {
        ...
        const checkDashboard = () => {
            ...
            // Check for different possible dashboard structures
            ...
            const hasAIAdvisorySection = document.querySelector('#ai-advisory-section');
            
            if (hasContent || hasAIAdvisorySection) {
    ...
    injectAdvisorySections() {
        ...
        // Check if AI advisory section already exists
        const existingSection = document.querySelector('#ai-advisory-section');
        if (existingSection) {
            console.log('✅ AI Advisory section already exists, setting up event listeners...');
            this.setupExistingAIAdvisorySection(existingSection);
            return;
        }
    ...
    setupExistingAIAdvisorySection(section) {
        console.log('🔧 Setting up existing AI Advisory section...');
        
        // Ensure AI Advisory UI is initialized
        if (!this.aiAdvisoryUI) {
            console.log('⚠️ AI Advisory UI not ready, initializing...');
            this.initializeAIAdvisoryUI();
        }
        
        // Ensure advisor manager is available
        if (!this.advisorManager) {
            console.log('⚠️ Advisor manager not ready, initializing...');
            this.initializeAIAdvisoryUI();
        }
        
        // Setup event listeners for the existing section (integration level)
        this.setupDetailViewEventListeners(section);
        
        // Initialize the AI Advisory UI content but NOT event listeners to avoid conflicts
        if (this.aiAdvisoryUI) {
            this.aiAdvisoryUI.initializeExistingSectionContent(section);
        }
        
        console.log('✅ Existing AI Advisory section setup completed');
    }
    
    initializeAIAdvisoryUI() {
        try {
            console.log('🔧 Initializing AI Advisory UI...');
            
            // Create advisor manager with fallback configuration
            const advisorManager = new AdvisorManager({
                azureOpenAI: {
                    endpoint: 'http://localhost:3000/api/ai-config',
                    deployment: 'Prod4',
                    model: 'gpt-4'
                },
                useSecureAPI: true
            ...
            // Store the advisor manager for use in event handlers
            this.advisorManager = advisorManager;
            
            // Initialize AI Advisory UI
            this.aiAdvisoryUI = new AIAdvisoryUI(advisorManager);
            
            console.log('✅ AI Advisory UI initialized successfully');
        } catch (error) {
            console.error('❌ Failed to initialize AI Advisory UI:', error);
        }
    }
    ...
    setupDetailViewEventListeners(section) {
        console.log('🔧 Setting up detail view event listeners...');
        ...
        if (generateBtn) {
            ...
            generateBtn.removeEventListener('click', this.generateDetailAnalysis);
            
            // Add new listener with proper error handling
            const handleGenerateClick = async () => {
                console.log('🔘 Generate button clicked (integration level)');
                try {
                    await this.generateDetailAnalysis();
                } catch (error) {
                    console.error('❌ Error in generate detail analysis:', error);
                }
            };
            
            generateBtn.addEventListener('click', handleGenerateClick);
            console.log('✅ Generate button event listener attached (integration)');
        }
        
        if (refreshBtn) {
            // Remove any existing listeners to prevent duplicates
            refreshBtn.removeEventListener('click', this.refreshDetailAnalysis);
            
            // Add new listener with proper error handling
            const handleRefreshClick = async () => {
                console.log('🔘 Refresh button clicked (integration level)');
                try {
                    await this.refreshDetailAnalysis();
                } catch (error) {
                    console.error('❌ Error in refresh detail analysis:', error);
                }
            };
            
            refreshBtn.addEventListener('click', handleRefreshClick);
            console.log('✅ Refresh button event listener attached (integration)');
        }
    }
